...
my %foreground = &read_proteins_and_peptides($foreground);
my %background = &read_proteins_and_peptides($background);
...
my $Nf = scalar(keys %foreground);
my $Nb = scalar(keys %background);
...
################################################################
# Given a source hash of scalar peptides and a pattern, add references
...
sub init_refhash {
    ...
    foreach my $protein (keys %{$source}) {
	foreach my $peptide (@{$source->{$protein}}){
	    if ($peptide =~ /$regex/){
		if (! defined $dest->{$pattern}->{$protein}){
		    $dest->{$pattern}->{$protein} = ();
		}
		push @{$dest->{$pattern}->{$protein}}, \$peptide;
	    }
	}
    }
}
...
sub search_paironce {
    ...
    foreach my $protein (keys %{$fg_by_pattern{$pattern}}){
	foreach my $peptide (@{$fg_by_pattern{$pattern}->{$protein}}){
	    if ($$peptide =~ /^$code$/) {
...
sub count_occurences(){
    ...
    foreach my $protein (keys %{$hash->{$parent}}){
	my $counted = 0;
	foreach my $peptide (@{$hash->{$parent}->{$protein}}) {
	    if ($$peptide =~ /$regex/) {
		$counted = 1;
		if (! defined $hash->{$pattern}->{$protein}){
		    $hash->{$pattern}->{$protein} = ();
		}
		push @{$hash->{$pattern}->{$protein}}, $peptide;
	    }
	}
	$count += $counted;
    }
...
##############################################################
sub read_proteins_and_peptides {
    my $input = shift;
    my %hash;
