...
def view_shared_subset(context, request, exp):
    ...
    if annotation_set_id is not None:
        params = "?annotation_set=%d" % (annotation_set_id)

        if not request.user.canViewAnnotations(annotation_set_id):
    ...
    subset_page = request.route_url('experiment_subset', id=exp.id) + params
    return {
