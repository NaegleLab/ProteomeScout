...
def fisher(a,b,c,d):
    ...
    key = (ar,br,cr,dr)
    if key in __cached:
        fishere = __cached[key]
    else:
        fishere = computeV2(ar,br,cr,dr)
        __cached[key] = fishere

    return restoreTable(a, ar,br,cr,dr, fishere)

def fisher_pvalue(observed_probability, a,b,c,d):
    ar, br, cr, dr = reduceTable(a,b,c,d)
    
    base_fisher = fisher(ar,br,cr,dr)
    pvalue = 0.0
...
if __name__ == "__main__":
    ...
    print "fisher fisher exact test a=%d, b=%d, c=%d, d=%d" % (a,b,c,d)
    val = fisher(a,b,c,d)
    print val

    print "pvalue for fisher exact test: ", fisher_pvalue(val, a,b,c,d)