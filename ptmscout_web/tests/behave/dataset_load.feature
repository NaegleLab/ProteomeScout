Feature: Dataset Load
	...
	Scenario: Load a correct dataset with degenerate methylation names
		  Given a user has loaded a dataset 
        ...
        Scenario: Load a dataset with bad amino acid assignments
		  Given a user has loaded a dataset
		  When the modification type does not match the amino acid for that species
		  Then the user should be sent an email which contains:
		  	| peptides | proteins | errors |
			| 9       | 9       | 3      |
	...
	Scenario: Bad acc
		Given a user submits a dataset 
		When there is an accession that looks like a GenPept accession, but is not a valid accession number
		Then the user should be sent an email with a link to the experiment which contains:
			| peptides | proteins | errors |
			| 17       | 14       | 1      |
	...
	Scenario: Multiple modification types in one peptide with assignment
		  Given a user submits a dataset
		  When a single peptide has more than one flavor of modification and mod_type assignments match the sequential order of modifcations
		  Then the user should be sent an email with a link to the experiment which contains:
		  	| peptides | proteins | errors |
			| 17       | 14       | 0      |
	...
	Scenario: Multiple modification types in one peptide without assignment
		  Given a user submits a dataset
		  When a single peptide has more than one flavor of modificationand the mod_type entry does not match the amino acids
		  Then the user should be sent an email with a link to the experiment which contains:
		  	| peptides | proteins | errors |
			| 16       | 13       | 1      |
	...
	Scenario: Handle an isoform
		  Given a user submits a dataset
		  When an isoform specific record is included
		  Then the user should be sent an email with a link to the experiment which contains:
		  	| peptides | proteins | errors |
			| 18       | 15       | 0      |
